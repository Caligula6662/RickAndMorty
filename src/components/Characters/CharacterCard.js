            ...
            <div className={"row no-gutters"}>
                ...
                <div className={"col-7"}>
                    <div className={"card-body h-100 d-flex flex-column justify-content-center"}>
                        <div>
                            <span className={`small ${helper.statusInfo(character.status)} py-1 px-2 text-white rounded-pill`}>{character.status}</span>
                            ...
                            <p className={"mb-0 small"}><span className={"font-weight-bold"}>Last Seen: </span>
                                {character.episode.map((episode, index, arr) => {
                                    if (arr.length - 1 === index) return <Link key={episode.id} to={`/episode/${episode.id}`}>{episode.name}</Link>
                                })}
                            </p>

                                <p className={"mb-0 small"}>
                                    <span className={"font-weight-bold"}>Last Location: </span>
                                    {character.location.name === "unknown"
                                        ? <span>{character.location.name}</span>
                                        : <Link to={`/location/${character.location.id}`}>{character.location.name}, {character.location.dimension}</Link>
                                    }
                                </p>
